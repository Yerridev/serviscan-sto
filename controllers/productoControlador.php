<?php
	...
	class productoControlador extends mainModel{
        ...
        public function registrar_producto_controlador(){
			...
			$img_dir='../views/assets/product/cover/';
					...
					$tabla.='
                        <div class="product-list mb-4">
                            <div class="product-list-img">
                                <figure>';
                                if(is_file("./views/assets/product/cover/".$rows['producto_portada'])){
                                    $tabla.='<img src="'.SERVERURL.'views/assets/product/cover/'.$rows['producto_portada'].'" class="img-fluid" alt="'.$rows['producto_nombre'].'" />';
                                }else{
                                    $tabla.='<img src="'.SERVERURL.'views/assets/product/cover/default.jpg" class="img-fluid" alt="'.$rows['producto_nombre'].'" />';
                                }
                                $tabla.='</figure>
                            </div>
		...
		public function agregar_galeria_producto_controlador(){
			...
			/*-- Directorios de imagenes - Image Directories --*/
			$img_dir='../views/assets/product/gallery/';
        ...
        public function eliminar_producto_controlador(){
			...
			if(is_file('../views/assets/product/cover/'.$campos['producto_portada'])){
				chmod('../views/assets/product/cover/', 0777);
				if(!unlink('../views/assets/product/cover/'.$campos['producto_portada'])){
                    $alerta=[
                        "Alerta"=>"simple",
                        "Titulo"=>"Ocurrió un error inesperado",
            ...
            if($check_imagen->rowCount()>0){
                while($rows=$check_imagen->fetch()){
                    ...
                    if($eliminar_imagen->rowCount()==1){
                        ...
                        /* Eliminando la imagen - Deleting the image */
                        if(is_file('../views/assets/product/gallery/'.$rows['imagen_nombre'])){
                            chmod('../views/assets/product/gallery/', 0777);
                            if(!unlink('../views/assets/product/gallery/'.$rows['imagen_nombre'])){
                                $alerta=[
                                    "Alerta"=>"simple",
                                    "Titulo"=>"Ocurrió un error inesperado",
